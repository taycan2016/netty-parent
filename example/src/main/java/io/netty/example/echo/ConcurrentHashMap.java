...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            ...
            // 如果sizectl == sc的话,通过原子cas为 sizeCtl : -1
            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                // 等于-1 表示已经有线程抢占初始化的权限
                try {
                    if ((tab = table) == null || tab.length == 0) {
                        ...
                        table = tab = nt;
                        // 设置sc的数值为当前容量大小的扩容因子的大小 12
                        // sizeCtl大于零表示下一次的扩容大小
                        sc = n - (n >>> 2);
                    }
                } finally {
                    sizeCtl = sc;
    ...
    private final void addCount(long x, int check) {
        ...
        if ((as = counterCells) != null ||
                //没有并发的情况下 直接设置basecount值
            !U.compareAndSwapLong(this, BASECOUNT, b = baseCount, s = b + x)) {
            CounterCell a; long v; int m;
            boolean uncontended = true;
            if (as == null || (m = as.length - 1) < 0 ||
                    //并发随机获取得随机数 与数组个数 获得结果落在元素个数的数组内  == null
        ...
        if (check >= 0) {
            ...
            //元素的个数大于扩容因子 table不为null table小于最大容量
            while (s >= (long)(sc = sizeCtl) && (tab = table) != null &&
                   (n = tab.length) < MAXIMUM_CAPACITY) {
                // 1000 0000 0001 1011高位补0
                // 0000 0000 0000 0000 1000 0000 0001 1011
                int rs = resizeStamp(n);
                if (sc < 0) { // sc小于0(不等于-1) 有n线程在扩容
                ...
                // 1000 0000 0001 1011 0000 0000 0000 0010左移16位
                // 高16位代表扩容的标记
                // 低16位代表扩容的线程数 2表示有一个线程
                // 1: 需要保存每次扩容的扩容戳都是唯一的
                // 2: 支持并发扩容
    ...
    private final void transfer(Node<K,V>[] tab, Node<K,V>[] nextTab) {
        ...
        // 16右移3位(除以8)=2 除以cpu的核心数小于16使用16
        // 表示每个cpu可以对16个长度的数组进行扩容 0-15 16-31
